...
final class NT_Content_Images_Audit_Admin {
	...
	public function register_menu(): void {
		add_submenu_page(
			'nt-content-images',
			__( 'Kiểm tra nội dung', 'nt-tao-anh-noi-dung-wordpress' ),
			__( 'Kiểm tra nội dung', 'nt-tao-anh-noi-dung-wordpress' ),
			'manage_options',
	...
	public function enqueue_assets(): void {
		...
		wp_enqueue_style( 'nt-content-images-audit-admin', NT_CONTENT_IMAGES_URL . 'admin/assets/audit-admin.css', array(), NT_CONTENT_IMAGES_VERSION );
		wp_enqueue_script( 'nt-content-images-audit-admin', NT_CONTENT_IMAGES_URL . 'admin/assets/audit-admin.js', array( 'wp-api-fetch' ), NT_CONTENT_IMAGES_VERSION, true );
		wp_localize_script(
			...
			array(
				...
				'exportUrl' => wp_nonce_url( admin_url( 'admin-post.php?action=nt_content_images_audit_export' ), 'nt_content_images_audit_export' ),
	...
	public function render_page(): void {
		...
		$allowed_types = $this->query->get_allowed_post_types();
		$labels        = $this->post_types->get_labels();
		?>
		<div class="wrap ntci-audit" id="ntci-audit-app">
			...
			<div class="notice notice-info inline"><p><?php echo esc_html__( 'Chức năng chỉ đọc dữ liệu. Các post type hiển thị được lấy từ Site Profile; plugin chưa tạo hoặc chèn ảnh.', 'nt-tao-anh-noi-dung-wordpress' ); ?></p></div>
			...
			<section class="ntci-panel" aria-labelledby="ntci-settings-title">
				...
				<div class="ntci-form-grid">
					<fieldset>
						...
						<?php foreach ( $allowed_types as $index => $post_type ) : ?>
							<label><input type="checkbox" name="post_types" value="<?php echo esc_attr( $post_type ); ?>" <?php checked( 0 === $index || in_array( $post_type, array( 'post', 'page' ), true ) ); ?>> <?php echo esc_html( $labels[ $post_type ] ?? $post_type ); ?> <code><?php echo esc_html( $post_type ); ?></code></label>
						<?php endforeach; ?>
					</fieldset>
					<fieldset>
					...
					<label><span><?php echo esc_html__( 'Chế độ quét', 'nt-tao-anh-noi-dung-wordpress' ); ?></span><select id="ntci-scan-mode"><option value="changed"><?php echo esc_html__( 'Nội dung mới và đã thay đổi', 'nt-tao-anh-noi-dung-wordpress' ); ?></option><option value="new"><?php echo esc_html__( 'Chỉ nội dung chưa audit', 'nt-tao-anh-noi-dung-wordpress' ); ?></option><option value="all"><?php echo esc_html__( 'Quét lại toàn bộ', 'nt-tao-anh-noi-dung-wordpress' ); ?></option></select></label>
					<label><span><?php echo esc_html__( 'Số nội dung mỗi batch', 'nt-tao-anh-noi-dung-wordpress' ); ?></span><select id="ntci-batch-size"><option value="10">10</option><option value="20" selected>20</option><option value="30">30</option><option value="50">50</option></select></label>
				</div>
			...
			<section class="ntci-panel" aria-labelledby="ntci-progress-title"><h2 id="ntci-progress-title"><?php echo esc_html__( 'Tiến độ', 'nt-tao-anh-noi-dung-wordpress' ); ?></h2><div class="ntci-progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><span id="ntci-progress-bar"></span></div><div class="ntci-progress-meta" id="ntci-progress-meta"><?php echo esc_html__( 'Chưa có tiến trình.', 'nt-tao-anh-noi-dung-wordpress' ); ?></div></section>

			<section aria-labelledby="ntci-summary-title"><h2 id="ntci-summary-title"><?php echo esc_html__( 'Tổng quan kết quả', 'nt-tao-anh-noi-dung-wordpress' ); ?></h2><div class="ntci-summary-grid" id="ntci-summary"></div></section>

			<section class="ntci-panel" aria-labelledby="ntci-results-title">
				<div class="ntci-section-heading"><h2 id="ntci-results-title"><?php echo esc_html__( 'Danh sách nội dung đã kiểm tra', 'nt-tao-anh-noi-dung-wordpress' ); ?></h2><a class="button" id="ntci-export" href="#"><?php echo esc_html__( 'Xuất CSV', 'nt-tao-anh-noi-dung-wordpress' ); ?></a></div>
				...
				<div class="ntci-table-wrap"><table class="widefat striped" id="ntci-results-table"><thead><tr><th><?php echo esc_html__( 'Nội dung', 'nt-tao-anh-noi-dung-wordpress' ); ?></th><th><?php echo esc_html__( 'Post type', 'nt-tao-anh-noi-dung-wordpress' ); ?></th><th><?php echo esc_html__( 'Ảnh đại diện', 'nt-tao-anh-noi-dung-wordpress' ); ?></th><th><?php echo esc_html__( 'Ảnh nội dung', 'nt-tao-anh-noi-dung-wordpress' ); ?></th><th><?php echo esc_html__( 'Số từ', 'nt-tao-anh-noi-dung-wordpress' ); ?></th><th><?php echo esc_html__( 'Ưu tiên', 'nt-tao-anh-noi-dung-wordpress' ); ?></th><th><?php echo esc_html__( 'Trạng thái', 'nt-tao-anh-noi-dung-wordpress' ); ?></th></tr></thead><tbody></tbody></table></div>
				<div class="ntci-pagination" id="ntci-pagination"></div>
			</section>

			<section class="ntci-panel" id="ntci-detail-panel" hidden aria-labelledby="ntci-detail-title"><div class="ntci-section-heading"><h2 id="ntci-detail-title"><?php echo esc_html__( 'Chi tiết nội dung', 'nt-tao-anh-noi-dung-wordpress' ); ?></h2><button type="button" class="button-link" id="ntci-detail-close"><?php echo esc_html__( 'Đóng', 'nt-tao-anh-noi-dung-wordpress' ); ?></button></div><div id="ntci-detail"></div></section>
		</div>
		<?php
	}
}
