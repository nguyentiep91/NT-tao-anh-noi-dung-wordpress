		...
		<div class="wrap ntci-generation" id="ntci-generation-app">
			...
			<section class="ntci-generation-panel">
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<p style="margin:10px 0 4px;"><strong><?php echo esc_html__( 'Các bố cục tham gia trộn:', 'nt-tao-anh-noi-dung-wordpress' ); ?></strong> <small><?php echo esc_html__( '(mẫu "Tối giản không tiêu đề" không hiện chữ nên chỉ tick khi muốn xen ảnh sạch)', 'nt-tao-anh-noi-dung-wordpress' ); ?></small></p>
					<p style="margin:4px 0 12px;">
						<?php foreach ( $overlay['templates'] as $template ) : ?>
							<label style="display:inline-block;margin:0 16px 6px 0;"><input type="checkbox" name="template_settings[mix_templates][]" value="<?php echo esc_attr( (string) $template['id'] ); ?>" <?php checked( in_array( $template['id'], (array) $overlay['settings']['mix_templates'], true ) ); ?>> <?php echo esc_html( (string) $template['label'] ); ?></label>
						<?php endforeach; ?>
					</p>
