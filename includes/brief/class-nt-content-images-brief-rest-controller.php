<?php
/**
 * REST controller for profile-aware image brief generation and workflow.
 *
...
final class NT_Content_Images_Brief_REST_Controller {
	...
	public function register_routes(): void {
		register_rest_route( self::NAMESPACE, '/briefs/generate', array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'generate' ), 'permission_callback' => array( $this, 'can_manage' ) ) );
		register_rest_route( self::NAMESPACE, '/briefs', array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( $this, 'get_list' ), 'permission_callback' => array( $this, 'can_manage' ) ) );
		register_rest_route( self::NAMESPACE, '/briefs/candidates', array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( $this, 'get_candidates' ), 'permission_callback' => array( $this, 'can_manage' ) ) );
		register_rest_route( self::NAMESPACE, '/briefs/(?P<id>\d+)', array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( $this, 'get_item' ), 'permission_callback' => array( $this, 'can_manage' ) ) );
		register_rest_route( self::NAMESPACE, '/briefs/(?P<id>\d+)/status', array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'update_status' ), 'permission_callback' => array( $this, 'can_manage' ) ) );
		register_rest_route( self::NAMESPACE, '/briefs/summary', array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( $this, 'get_summary' ), 'permission_callback' => array( $this, 'can_manage' ) ) );
	}
	...
	public function generate( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		if ( empty( $post_ids ) ) {
			return new WP_Error( 'ntci_brief_missing_posts', __( 'Chưa chọn nội dung để tạo kế hoạch hình ảnh.', 'nt-tao-anh-noi-dung-wordpress' ), array( 'status' => 400 ) );
		}
		...
		return rest_ensure_response(
			$this->repository->get_list(
				array(
					...
					'post_type_mapping' => sanitize_key( (string) $request->get_param( 'post_type_mapping' ) ),
				)
			)
		);
	}
	...
	public function get_candidates( WP_REST_Request $request ): WP_REST_Response {
		...
		$enabled = array_values( array_map( 'sanitize_key', (array) ( $profile['enabled_post_types'] ?? array( 'post', 'page' ) ) ) );
		...
		foreach ( $result['items'] as $item ) {
			if ( ! in_array( sanitize_key( (string) $item['post_type'] ), $enabled, true ) ) {
				continue;
			}
	...
	public function update_status( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		if ( 'approved' === $status ) {
			if ( 'invalid' === (string) $item['validation_status'] || 'outdated' === (string) $item['status'] ) {
				return new WP_Error( 'ntci_brief_invalid', __( 'Không thể duyệt brief không hợp lệ hoặc đã lỗi thời.', 'nt-tao-anh-noi-dung-wordpress' ), array( 'status' => 400 ) );
			}
			$context = $this->profiles->get_context();
			if ( ! hash_equals( (string) $context['profile_hash'], (string) ( $item['profile_hash'] ?? '' ) ) || ! hash_equals( (string) $context['brand_hash'], (string) ( $item['brand_profile_hash'] ?? '' ) ) ) {
				return new WP_Error( 'ntci_brief_profile_outdated', __( 'Brief được tạo bằng cấu hình cũ. Hãy tạo lại trước khi duyệt.', 'nt-tao-anh-noi-dung-wordpress' ), array( 'status' => 409 ) );
			}
		}
